...
func TestResolveBuiltinsInheritThroughPrimary(t *testing.T) {
	...
	explore, err := cfg.Resolve("explore", ResolveContext{})
	if err != nil {
	...
	plan, err := cfg.Resolve("plan", ResolveContext{Home: "/home/me", ProjectID: "project-1"})
	if err != nil {
		t.Fatalf("Resolve(plan): %v", err)
	}
	if plan.Model != "test/base-model" {
		t.Fatalf("plan model = %q, want primary inheritance", plan.Model)
	}
	if want := filepath.Join("/home/me", ".lightcode", "projects", "project-1", "plans"); plan.WriteDir != want {
		t.Fatalf("plan write dir = %q, want project plans dir", plan.WriteDir)
	}
	if !contains(plan.Tools, submitPlanTool) {
		t.Fatalf("plan tools = %v, want submit_plan", plan.Tools)
	}
}
...
func TestBuiltinsExposeConcreteRootAndCodeOnlyCompact(t *testing.T) {
	...
	primary, err := cfg.Resolve("primary", ResolveContext{})
	if err != nil {
	...
	compactAgent, err := cfg.Resolve("compact", ResolveContext{})
	if err != nil {
...
func TestToolsPresenceControlsInheritanceEmptyAndSubset(t *testing.T) {
  ...
  "reader": {"tools": ["read_file", "run_command"]}
	...
	empty, err := cfg.Resolve("empty", ResolveContext{})
	if err != nil {
	...
	reader, err := cfg.Resolve("reader", ResolveContext{})
	if err != nil {
	...
	review, err := cfg.Resolve("review", ResolveContext{})
	if err != nil {
...
func TestWriteModelUpdatesAgentsJSON(t *testing.T) {
	...
	primary, _ := cfg.Resolve("primary", ResolveContext{})
	custom, _ := cfg.Resolve("custom", ResolveContext{})
